...
def init_db():
    try:
        ...
        # Migration: add initial_quantity if missing
        cursor.execute("PRAGMA table_info(hardware)")
        hw_cols = [col[1] for col in cursor.fetchall()]
        if "initial_quantity" not in hw_cols:
            cursor.execute("ALTER TABLE hardware ADD COLUMN initial_quantity INTEGER NOT NULL DEFAULT 0")
            # Back-fill: for existing rows, set initial_quantity = current quantity
            # (best approximation since we have no history)
            cursor.execute("UPDATE hardware SET initial_quantity = quantity WHERE initial_quantity = 0")
            logger.info("Migration: added initial_quantity to hardware")
...
def catalog():
    ...
    if search:
        query += " AND LOWER(h.item_name) LIKE ?"
    ...
    cursor.execute("SELECT SUM(quantity) FROM hardware")
...
def inventory_update(item_id):
    ...
    if added > 0:
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        cursor.execute(
            "INSERT INTO quantity_additions (item_id, added_quantity, timestamp) VALUES (?, ?, ?)",
...
def my_borrows():
    ...
    cursor.execute("""
        SELECT br.request_id, h.item_name, br.quantity, br.timestamp, br.status
        FROM borrow_requests br JOIN hardware h ON h.item_id = br.item_id
        WHERE br.username = ? AND br.status = 'pending'
        ORDER BY br.request_id DESC
    """, (username,))
    ...
    cursor.execute("""
        SELECT br.request_id, h.item_name, br.quantity, br.timestamp, br.status
        FROM borrow_requests br JOIN hardware h ON h.item_id = br.item_id
        WHERE br.username = ? AND br.status IN ('returned', 'rejected')
        ORDER BY br.request_id DESC
        LIMIT 50
    """, (username,))
